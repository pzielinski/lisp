...
(deftest do-eval-test 
     (let [env (setup-environment global-primitive-procedure-impl-map (the-empty-environment))]
       ...
       (let [e1 (extend-environment-with-map '{} env)]
         (is (= 2 (lookup-variable-value-in-env 'x (do-eval '(set! x 2) e1))))
         (is (= 3 (lookup-variable-value-in-env 'y (do-eval '(set! y (+ 1 2)) e1))))
         ;(is (= 5 (lookup-variable-value-in-env 'x (do-eval '(set! x (+ x y)) e1))));!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       )
       ;definition
       (let [e1 (extend-environment-with-map '{} env)]
         (is (= 2 (lookup-variable-value-in-env 'x (do-eval '(define x 2) e1))))
         (is (= 3 (lookup-variable-value-in-env 'y (do-eval '(define y (+ 1 2)) e1))))
         ;(is (= 5 (lookup-variable-value-in-env 'x (do-eval '(define x (+ x y)) e1))));!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
         ;(is (= 6 (do-eval '(doubler 3) (do-eval '(define doubler (lambda (x) (+ x x))) e1))));!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       )
       ;definition of function
       (let [e1 (extend-environment-with-map '{} env)]
         ;(is (not (nil? (lookup-variable-value-in-env 'doubler (do-eval '(define (doubler x) (+ x x)) e1)))));!!!!!!!!!!!!!!!!!!!!!
         ;(is (= 6 (lookup-variable-value-in-env 'y (do-eval '(define y (doubler 3)) e1))));!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       )
       ;if
